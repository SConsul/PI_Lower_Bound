...
def solve_adv(num_states, num_actions, reward_function, transition_function, discount_factor, type_mdp):
    ...
    while optimized is False:
        ...
        next_policy, improved_states,new_mode,sp_ind,sp_c,done_flag = get_next_policy(policy.copy(), num_actions,mode,sp_ind,sp_c,done_flag)
        ...
        if not np.all(check):
            ...
            policy = next_policy
        print("Current:", policy, "Next: ", next_policy, "Mode:", mode, "Num Iter: ", num_iterations)
...
def get_next_policy(curr_policy, num_actions,mode,sp_ind,sp_c,done_flag):
    improved_states = []
    num_states = len(curr_policy)
    # print("LALA",num_states)
    if mode =="special":
        if sp_ind == num_states:
            done_flag = True  
            print("DONE")
        elif sp_c ==0:
            index = sp_ind
            ...
            improved_states.append(index)
            sp_c +=1
        return curr_policy, improved_states, mode, sp_ind, sp_c,done_flag
    elif mode == "normal":
            ...
            if curr_policy[0] == num_actions-1:
                ...
                for sp_ind in range(num_states):
                    if curr_policy[sp_ind] != num_actions-1:
                        sp_ind+=1
                        break
        return curr_policy, improved_states,mode,sp_ind,0,done_flag